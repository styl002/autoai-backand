...
app.post("/chat", async (req, res) => {
  try {
    ...
    const summary = listings.slice(0, 8).map(c =>
      `• ${c.make} ${c.model} (${c.year}, ${(c.km||0).toLocaleString("de-DE")} km, ${c.fuel}, ${c.power} PS) → ${(c.price||0).toLocaleString("de-DE")} € [${(c.valueRating||"ok").toUpperCase()}]`
    ).join("\n");
    const system = `Du bist AutoBerater, KFZ-Experte. Echte Inserate:\n${summary||"Keine Suche."}\nAntworte auf Deutsch, max. 120 Wörter, kein Markdown.`;
    const messages = [
...
app.post("/verdict", async (req, res) => {
  try {
    const c = req.body;
    const text = await groqCall(
      "KFZ-Experte. Max. 60 Wörter Preis-Leistungs-Einschätzung auf Deutsch. Keine Formatierung.",
      `${c.make} ${c.model} ${c.variant||""}, Bj.${c.year}, ${(c.km||0).toLocaleString("de-DE")} km, ${c.fuel}, ${c.power} PS → ${(c.price||0).toLocaleString("de-DE")} €. Marktschnitt: ${(c.marketAvg||0).toLocaleString("de-DE")} €. Bewertung: ${c.valueRating}.`,
      200
    );
