"use strict";
const puppeteer=require("puppeteer-extra"),Stealth=require("puppeteer-extra-plugin-stealth");
puppeteer.use(Stealth());
const MAKE_IDS={"Audi":"1900","BMW":"3500","Mercedes-Benz":"17200","Volkswagen":"25200","Ford":"9000","Toyota":"24100","Tesla":"23603","Porsche":"20000","Opel":"19000","Skoda":"21700","Seat":"21500","Hyundai":"12100","Kia":"13300","Renault":"20900","Peugeot":"19700","Volvo":"25100","Mazda":"16700","Honda":"11600","Nissan":"18700","Dacia":"6050","Fiat":"8600","Alfa Romeo":"100","Jeep":"12900","Land Rover":"14100","Mini":"18000","Ferrari":"8800","Lamborghini":"13400","Maserati":"16200","BYD":"3000","Genesis":"9340","Cupra":"5765","MG":"17100","Polestar":"20050","Mitsubishi":"18400","Subaru":"22700","Suzuki":"22900","Smart":"22300","Jaguar":"12500"};
const BODY_MAP={"SUV":"SUV/Gelaendewagen/Pickup","Limousine":"Limousine","Kombi":"Kombi","Coupé":"Sportwagen/Coupe","Cabriolet":"Cabrio/Roadster","Kleinwagen":"Kleinwagen","Van":"Van/Minibus","Pickup":"SUV/Gelaendewagen/Pickup","Transporter":"Transporter"};
const FUEL_MAP={"Benzin":"PETROL","Diesel":"DIESEL","Elektro":"ELECTRICITY","Hybrid":"HYBRID","Plug-in-Hybrid":"PLUG_IN_HYBRID","Mild-Hybrid":"MILD_HYBRID","Erdgas":"NATURAL_GAS","LPG":"LPG","Wasserstoff":"HYDROGEN"};
...
async function scrapeMobile(f){
  ...
  if(f.minP)p.set("minPrice",f.minP);if(f.maxP)p.set("maxPrice",f.maxP);
  if(f.minKm)p.set("minMileage",f.minKm);if(f.maxKm)p.set("maxMileage",f.maxKm);
  ...
  if(f.radius&&f.radius!=="Alle")p.set("maxMilesRadius",f.radius);
  ...
  try{
    ...
    return await page.evaluate(()=>{
      ...
      return[...document.querySelectorAll('[data-testid="result-list-item"]')].slice(0,50).map((card,i)=>{
        const title=q(card,"h2")||q(card,'[data-testid="ad-title"]'),parts=title.split(" ");
        return{id:`mob-${i}-${Date.now()}`,source:"mobile.de",make:parts[0]||"–",model:parts.slice(1,3).join(" ")||"–",variant:parts.slice(3).join(" ")||"",
          price:num(q(card,'[data-testid="price-label"]')),km:num(q(card,'[data-testid="mileage"]')),
          year:yr(q(card,'[data-testid="first-registration"]')),fuel:q(card,'[data-testid="fuel-type"]')||"–",
          power:ps(q(card,'[data-testid="power"]')),gearbox:q(card,'[data-testid="transmission"]')||"–",
          location:q(card,'[data-testid="seller-info-location"]')||"Deutschland",seller:q(card,'[data-testid="seller-name"]')||"–",
          url:card.querySelector("a")?.href||"",img:card.querySelector("img")?.src||"",
          body:"–",color:"–",drive:"–",doors:4,seats:5,owners:1,consumption:"–",co2:"–",emission:"–",
          inspected:false,guarantee:false,financing:false,features:[],marketAvg:0,valueRating:"ok",valueNote:"",description:""};
      });
    });
